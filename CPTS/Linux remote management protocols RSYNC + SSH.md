...
the sshd_config file is responsible for the openSSH server, has only a few settings by default. However the default config includes x11 forwarding which has a command inject vuln in version 7.2p1 of OpenSSH. 
...
```shell-session
JackTheWizard@htb[/htb]$ git clone https://github.com/jtesta/ssh-audit.git && cd ssh-audit
JackTheWizard@htb[/htb]$ ./ssh-audit.py 10.129.14.132
...
```shell-session
JackTheWizard@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132
...
```shell-session
JackTheWizard@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132 -o PreferredAuthentications=password
...
```shell-session
JackTheWizard@htb[/htb]$ cat /etc/ssh/sshd_config  | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
JackTheWizard@htb[/htb]$ sudo nmap -sV -p 873 127.0.0.1
...
```shell-session
JackTheWizard@htb[/htb]$ nc -nv 127.0.0.1 873
...
```shell-session
JackTheWizard@htb[/htb]$ rsync -av --list-only rsync://127.0.0.1/dev
...
```shell-session
JackTheWizard@htb[/htb]$ cat /etc/hosts.equiv
...
```shell-session
JackTheWizard@htb[/htb]$ sudo nmap -sV -p 512,513,514 10.0.17.2
...
```shell-session
JackTheWizard@htb[/htb]$ cat .rhosts
...
```shell-session
JackTheWizard@htb[/htb]$ rlogin 10.0.17.2 -l htb-student
...
```shell-session
JackTheWizard@htb[/htb]$ rusers -al 10.0.17.5
