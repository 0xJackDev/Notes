...
Virtual hosting is the ability of web servers to distinguish multiple websites at the same IP address. this is done by leveraging the HTTP host header
...
```shell-session
JackTheWizard@htb[/htb]$ gobuster vhost -u http://<target_IP_address> -w <wordlist_file> --append-domain
```
...
```shell-session
[!bash!]$ gobuster vhost -u http://inlanefreight.htb -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-110000.txt --app
```

VHOSTS are creating at the Web server level AKA the nginx config while subdomains are made at the DNS level
